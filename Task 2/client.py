...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa fii conectat pentru a folosi PUBLISH.")
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE LOCALA: Lipseste mesajul. Format corect: PUBLISH <text_mesaj>")
            continue
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa fii conectat pentru a folosi DELETE.")
            continue
        if len(parti) < 2 or not parti[1].strip().isdigit():
            print("EROARE LOCALA: ID-ul este invalid. Format corect: DELETE <numar_intreg>")
            continue
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa fii conectat pentru a folosi LIST.")
            continue
